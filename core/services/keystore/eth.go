...
func (ks *eth) addKey(address common.Address, chainID *big.Int, qopts ...pg.QOpt) error {
	...
	sql := `INSERT INTO evm_key_states (address, disabled, evm_chain_id, created_at, updated_at)
...
func (ks *eth) XXXTestingOnlySetState(state ethkey.State) {
	...
	*existingState = state
	sql := `UPDATE evm_key_states SET address = :address, is_disabled = :is_disabled, evm_chain_id = :evm_chain_id, updated_at = NOW()
...
func (ks *eth) addKeyState(key ethkey.KeyV2, chainID *big.Int, isDisabled bool) (err error) {
	ks.lock.Lock()
	defer ks.lock.Unlock()
...
VALUES ($1, $2, $3, NOW(), NOW()) RETURNING *;`
		if err = ks.orm.q.Get(state, sql, key.Address, isDisabled, chainID); err != nil {
			return errors.Wrap(err, "failed to insert evm_key_state")
		}
